...
> - **(3a) PoC focado antes de PR4** — validar Opção 3 com 3 collectors representativos (log/audit/container) em ovh-spark antes de implementar `ServerUpgradeService.upgrade()`.
>
...
| 5 | ovh-spark | 54.36.100.35 | ubuntu | sudo NOPASSWD: ALL | ❌ não instalado |
| 6 | ovh-automabothub | 51.79.84.65 | ubuntu | sudo NOPASSWD: ALL | ❌ não instalado |
| 7 | synthfin | 192.99.43.163 | ubuntu | (disabled — ignorar) | — |

**Acesso SSH local** já está em `~/.ssh/config` (aliases: `hetzner`, `ovh-spark`, `ovh-automabothub`, `synthfin`). Permite ao Guardian Central executar upgrade remoto sem intervenção manual no servidor.
...
Como você confirmou que tem acesso SSH a todos os servidores via aliases (`hetzner`, `ovh-spark`, `ovh-automabothub`), o fluxo é:
...
✅ PR3  — PoC v2 em ovh-spark: 26/26 testes passando; 3 collectors representativos
...
2. /upgrade-server 5  (ovh-spark)        — 10 min
3. /upgrade-server 6  (ovh-automabothub) — 10 min
...
| PR3  | **PoC v2** em ovh-spark — 26/26 testes passando; 3 collectors representativos provam Opção 3 | ✅ mergeado 2026-05-31 | 1h |
...
| **PR7** | **Migração real ovh-spark + ovh-automabothub (janela de manutenção)** | ⏳ | 30min |
...
### Antes (legacy)
```
Usuário no Telegram: /add-server ovh-spark 54.36.100.35 49222 ubuntu /root/.ssh/id_ed25519
Guardian: testa SSH → cadastra → fim
```

### Depois (Tier 0)
```
Usuário no Telegram: /add-server ovh-spark 54.36.100.35 49222
...
- ✅ PR2/PR3 mergeados — guardian-shell.sh v2 + PoC v2 em ovh-spark 26/26 testes
