...
const VERSION_TS = resolve(REPO_ROOT, 'src/security/guardian-shell-version.ts');
...
function main(): void {
  ...
  // Versão estável baseada no conteúdo do allowlist — dois builds com o mesmo
  // allowed-commands.txt produzem o mesmo shellVersion, evitando reinstalls
  // desnecessários quando apenas o timestamp do build muda.
  const shellVersion = sha256(entries.map((e) => e.hash).join('\n')).slice(0, 16);
  ...
  // Substitui VERSION no guardian-shell.sh com o shellVersion (hash do conteúdo).
  const freshShell = readFileSync(SHELL_PATH, 'utf-8');
  const shellWithVersion = freshShell.replace(
    /# BEGIN_VERSION\nVERSION='[^']*'\n# END_VERSION/,
    `# BEGIN_VERSION\nVERSION='${shellVersion}'\n# END_VERSION`,
  );
  if (shellWithVersion !== freshShell) {
    writeFileSync(SHELL_PATH, shellWithVersion, 'utf-8');
    console.log(`[generate-allowlist] updated VERSION in ${SHELL_PATH}`);
  }

  // Copia o guardian-shell.sh para dist/security/ para ficar disponível
  // no container Docker (que só copia dist/ na imagem final).
  const distShellPath = resolve(REPO_ROOT, 'dist/security/guardian-shell.sh');
  copyFileSync(SHELL_PATH, distShellPath);
  console.log(`[generate-allowlist] copied guardian-shell.sh to ${distShellPath}`);

  // Gera guardian-shell-version.ts para o worker importar em runtime.
  writeFileSync(
    VERSION_TS,
    `// AUTO-GENERATED by scripts/generate-allowlist.ts — do not edit manually\n` +
    `export const EXPECTED_SHELL_VERSION = '${shellVersion}';\n`,
    'utf-8',
  );
  console.log(`[generate-allowlist] wrote ${VERSION_TS}`);
