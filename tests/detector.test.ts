...
describe('EventDetector', () => {
  ...
  it('detects password login via rawLog', () => {
    ...
    const events: NormalizedEvent[] = [
      makeEvent({
        eventType: 'ssh_login_success',
        sourceIp: '177.37.108.216',
        rawLog: 'Accepted password for ubuntu from 177.37.108.216 port 54321 ssh2',
      }),
    ];
    const detected = EventDetector.detect(events);
  ...
  it('does NOT flag key-based login as password login', () => {
    const events: NormalizedEvent[] = [
      makeEvent({
        eventType: 'ssh_login_success',
        sourceIp: '177.37.108.216',
        rawLog: 'Accepted publickey for ubuntu from 177.37.108.216 port 54321 ssh2',
      }),
    ];
    const detected = EventDetector.detect(events);
  ...
  it('does NOT flag trusted IPs as unauthorized', () => {
    const events: NormalizedEvent[] = [
      makeEvent({
        eventType: 'ssh_login_success',
        sourceIp: '177.37.108.216',
        rawLog: 'Accepted publickey for ubuntu from 177.37.108.216',
      }),
    ];
    const detected = EventDetector.detect(events);
